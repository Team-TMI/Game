...
bool FIPCHandler::ReceiveMessage()
{
	...
	if (ReadFile(Pipe, Buffer, BufferSize, &BytesRead, NULL))
	{
		...
		std::memcpy(CachedBuffer + CachedLength, Buffer, BytesRead);
		...
		std::memcpy(&Header, CachedBuffer, sizeof(FMessageHeader));
		...
		std::memcpy(&Message, CachedBuffer, Header.PayloadSize);
